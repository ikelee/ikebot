...
export class SimpleResponderAgent extends Agent {
  ...
  async execute(input: AgentInput, context: AgentExecutionContext): Promise<AgentOutput> {
    ...
    try {
      ...
      // Build messages array - just the current prompt, no history for simple queries
      ...
      // Model from context (runAgentFlow passes resolved model for simple tier)
      ...
      // Direct model completion using completeSimple (fastest path)
  ...
  protected buildSystemPrompt(input: AgentInput): string {
    // Extract user timezone from context if available
    const userTimezone = (input.context?.userTimezone as string) || "UTC";
