...
describe("trigger handling", () => {
  ...
  it("injects group activation context into the system prompt", async () => {
    await withTempHome(async (home) => {
      ...
      const res = await getReplyFromConfig(
        {
          Body: "run a quick script to list the current directory",
          From: "123@g.us",
          To: "+2000",
          ChatType: "group",
          Provider: "whatsapp",
  ...
  it("handles bare /new without running a model", async () => {
    await withTempHome(async (home) => {
      const res = await getReplyFromConfig(
        {
          Body: "/new",
  ...
  it("handles /new with arguments without running a model", async () => {
    await withTempHome(async (home) => {
      const res = await getReplyFromConfig(
        {
          Body: "/new take notes",
