...
import type { TypingMode } from "../../../../../infra/config/types.js";
...
import { resolveAgentModelFallbacksOverride } from "../../../../../runtime/agent-scope.js";
...
} from "../../reply-building/reply-payloads.js";
import { resolveReplyToMode } from "../../reply-building/reply-threading.js";
import { createTypingSignaler } from "../../streaming/typing-mode.js";
import {
  incrementRunCompactionCount,
  persistRunSessionUsage,
} from "../session/session-run-accounting.js";
...
export function createFollowupRunner(params: {
  ...
  return async (queued: FollowupRun) => {
    try {
      ...
      if (shouldLogVerbose() && payloadArray.length > 0) {
        const channel = queued.run.messageProvider ?? "?";
        const snippet = payloadArray
          .map((p) => (p.text ?? "").trim().slice(0, 200))
          .filter(Boolean)
          .join(" | ");
        if (snippet) {
          logVerbose(
            `[${channel}] reply from model: ${snippet}${snippet.length >= 200 ? "..." : ""}`,
          );
        }
      }
