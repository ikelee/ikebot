...
var A2uiMessageProcessor = class A2uiMessageProcessor {
  ...
  #convertKeyValueArrayToMap(arr) {
    ...
    for (const item of arr) {
      if (!isObject$1(item) || !("key" in item)) {
        continue;
      }
      const key = item.key;
  ...
  class extends i$5 {
    ...
    update(s, [i]) {
      if (void 0 === this.st) {
        ...
        for (const t in i) {
          i[t] && !this.nt?.has(t) && this.st.add(t);
        }
