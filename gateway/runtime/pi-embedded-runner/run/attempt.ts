...
import { resolveTelegramInlineButtonsScope } from "../../../entrypoints/telegram/inline-buttons.js";
...
import { buildTtsSystemPromptHint } from "../../../runtime/tts/tts.js";
...
function logLongContent(
  logFn: (msg: string) => void,
  prefix: string,
  text: string,
  fullInVerbose: boolean,
): void {
  const t = text?.trim() ?? "";
  ...
  try {
    ...
    try {
      ...
      try {
        ...
        const sysLen = systemPromptText?.length ?? 0;
        const msgCount = activeSession.messages.length;
        log.info(
          `model input: ${params.provider}/${params.modelId} systemPrompt=${sysLen} chars messages=${msgCount} userPrompt=${effectivePrompt.length} chars`,
        );
        logLongContent(log.info.bind(log), "model input system", systemPromptText ?? "", true);
        logLongContent(log.info.bind(log), "model input user", effectivePrompt, true);
        ...
        const lastAssistant = messagesSnapshot
          .slice()
          .toReversed()
          .find((m) => m.role === "assistant") as { content?: unknown } | undefined;
        const responseText =
          lastAssistant && Array.isArray(lastAssistant.content)
            ? extractAssistantText(lastAssistant as Parameters<typeof extractAssistantText>[0])
            : assistantTexts.join("");
