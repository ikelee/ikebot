...
import type { GatewayAuthMode } from "../../../infra/config/config.js";
import type { GatewayWsLogStyle } from "../../../server/ws-logging.js";
...
import { setGatewayWsLogStyle } from "../../../server/ws-logging.js";
import { writeVerboseSentinel } from "../../../verbose-sentinel.js";
...
async function runGatewayCommand(opts: GatewayRunOpts) {
  // Commander may attach --verbose to the parent "gateway" command for "gateway run --verbose"
  const verbose = Boolean(opts?.verbose) || process.argv.includes("--verbose");
  if (verbose) {
    setVerbose(true);
    writeVerboseSentinel();
    process.stderr.write("[gateway] verbose sentinel written; full model I/O will be logged.\n");
  }
