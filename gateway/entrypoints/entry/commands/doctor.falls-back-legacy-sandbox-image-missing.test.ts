...
vi.mock("../runtime/skills-status.js", () => ({
...
vi.mock("../server/call.js", async (importOriginal) => {
  const actual = await importOriginal<typeof import("../../server/call.js")>();
  return {
    ...actual,
    callGateway,
  };
...
vi.mock("../runtime/auth-profiles.js", async (importOriginal) => {
  const actual = await importOriginal();
  return {
    ...actual,
...
vi.mock("./doctor-state-migrations.js", () => ({
  ...
  detectLegacyStateMigrations: vi.fn().mockResolvedValue({
    ...
    sessions: {
      ...
      targetDir: "/tmp/state/runtime/main/sessions",
      targetStorePath: "/tmp/state/runtime/main/sessions/sessions.json",
      hasLegacy: false,
      legacyKeys: [],
    },
    agentDir: {
      legacyDir: "/tmp/state/agent",
      targetDir: "/tmp/state/runtime/main/agent",
      hasLegacy: false,
    },
    whatsappAuth: {
      legacyDir: "/tmp/oauth",
