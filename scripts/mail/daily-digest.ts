...
type DigestModelOutput = {
  summary: string;
  importantItems: Array<{ title: string; why: string; action?: string }>;
  actionItems: string[];
  noiseSummary: string;
};
...
function safeJsonParse<T>(line: string): T | undefined {
  try {
...
async function collectRecentRecordIds(
  eventsFile: string,
  startIso: string,
  endIso: string,
): Promise<Set<string>> {
  ...
  for await (const line of rl) {
    ...
    const event = safeJsonParse<MailEvent>(line);
...
async function loadRecordsById(recordsFile: string, ids: Set<string>): Promise<MailRecord[]> {
  ...
  for await (const line of rl) {
    ...
    const record = safeJsonParse<MailRecord>(line);
    if (!record || !record.id) {
      continue;
    }
...
function compactRecord(record: MailRecord): string {
...
async function summarizeWithQwen(
  ...
  const parsed = safeJsonParse<DigestModelOutput>(raw);
